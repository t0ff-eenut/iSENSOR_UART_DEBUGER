...
def graph_data_setting():
    global A_graph_handle, A_graph_data_handle, A_graph_occu_handle, i_tp1
    # 각 그래프마다 Data 공간 할당
    try:
        while True:
            if not Q_adc_graph_data_buffer.empty():
                ...
                # print("Q_adc_graph_data_buffer A_receive_data : ", A_receive_data)
                ...
                for i_curve_data_index in range(C_CURVE_DEFINE.GRAPH_START, C_CURVE_DEFINE.CURVE_DEFINE_ARRAY_SIZE, 1):
                    # print("A_receive_data[i_curve_data_index] : ", A_receive_data[i_curve_data_index])
                    A_graph_handle[C_GRAPH_DEFINE.ADC_GRAPH].A_data_windows[i_curve_data_index] = A_receive_data[i_curve_data_index]
                    # print("A_graph_handle[C_GRAPH_DEFINE.ADC_DELTA_GRAPH].A_data_windows[i_curve_data_index] : ", A_graph_handle[C_GRAPH_DEFINE.ADC_GRAPH].A_data_windows[i_curve_data_index])
                    # print("")

            if not Q_adc_delta_graph_data_buffer.empty():
                # [DATA[], MIN[], MAX[], MID[], MEAN[], MIN_MAX_CENTER[], OTSU[]]
                A_receive_data = Q_adc_delta_graph_data_buffer.get()
                # print("Q_adc_delta_graph_data_buffer A_receive_data : ", A_receive_data)
                # for i_curve_data_index in range(C_CURVE_DEFINE.GRAPH_START, C_CURVE_DEFINE.NOMAL_GRAPH_SIZE, 1):
