...
######
# def otsu(graph_sel):
#     global A_adc_graph_object, A_adc_delta_graph_object, A_adc_delta_graph_bg_data

#     A_i_adc_val_count = [0] * d_ADC_MAX_RANGE
#     i_data_len = 0

#     if graph_sel == C_GRAPH_DEFINE.ADC_GRAPH:
#         i_data_len = len(A_adc_graph_object[C_CURVE_DEFINE.DATA_CURVE])
#         for i_data_sel in range(i_data_len):
#             A_i_adc_val_count[A_adc_graph_object[C_CURVE_DEFINE.DATA_CURVE][i_data_sel]] += 1
#     elif graph_sel == C_GRAPH_DEFINE.ADC_DELTA_GRAPH:
#         i_data_len = len(A_adc_delta_graph_object[C_CURVE_DEFINE.DATA_CURVE])
#         for i_data_sel in range(i_data_len):
#             A_i_adc_val_count[A_adc_delta_graph_object[C_CURVE_DEFINE.DATA_CURVE][i_data_sel]] += 1
#         ######
...
#     global d_double, A_adc_graph_object, A_adc_delta_graph_object
...
#     if graph_sel == C_GRAPH_DEFINE.ADC_GRAPH:
#         A_temp = A_adc_graph_object[C_CURVE_DEFINE.DATA_CURVE]

#     elif graph_sel == C_GRAPH_DEFINE.ADC_DELTA_GRAPH:
#         A_temp = A_adc_delta_graph_object[C_CURVE_DEFINE.DATA_CURVE]
...
def data_process():
    global d_double, i_moving_std_occu_count, i_moving_std_noise_count, A_adc_graph_object, A_adc_delta_graph_object, A_adc_delta_graph_bg_data

    # 정의된 Graph 수를 파악하고, 정의가 안되어 있으면 Graph 공간 생성
    ...
    while True:
        if not Q_data_buffer.empty():
            # print("len DATA_CURVE : ", len(A_adc_graph_object[C_CURVE_DEFINE.DATA_CURVE]))
            ...
            if len(A_adc_graph_object[C_CURVE_DEFINE.DATA_CURVE]) > d_WINDOW_SIZE:
                ...
                for i_curve_index in range(C_CURVE_DEFINE.DATA_CURVE, C_CURVE_DEFINE.CURVE_DEFINE_ARRAY_SIZE, 1):
                    # A_adc_graph_object[i_curve_index] = A_adc_graph_object[i_curve_index][1:d_WINDOW_SIZE]
            ...
            # A_adc_graph_object[C_CURVE_DEFINE.MIN_CURVE].append(int(min(A_adc_graph_object[C_CURVE_DEFINE.DATA_CURVE])))
            # A_adc_graph_object[C_CURVE_DEFINE.MAX_CURVE].append(int(max(A_adc_graph_object[C_CURVE_DEFINE.DATA_CURVE])))
            # A_adc_graph_object[C_CURVE_DEFINE.MID_CURVE].append(int(median(A_adc_graph_object[C_CURVE_DEFINE.DATA_CURVE])))
            # A_adc_graph_object[C_CURVE_DEFINE.MEAN_CURVE].append(int(mean(A_adc_graph_object[C_CURVE_DEFINE.DATA_CURVE])))
            # A_adc_graph_object[C_CURVE_DEFINE.MIN_MAX_CENTER_CURVE].append(((int(min(A_adc_graph_object[C_CURVE_DEFINE.DATA_CURVE])) + int(max(A_adc_graph_object[C_CURVE_DEFINE.DATA_CURVE]))) / 2))
            # A_i_otsu_resualt = otsu(C_GRAPH_DEFINE.ADC_GRAPH)
            # A_adc_graph_object[C_CURVE_DEFINE.OTSU_CURVE].append(A_i_otsu_resualt[C_OTSU_RETURN_DEFINE.MAX_TH])
            # A_i_moving_avg_std_resualt = moving_avg_std_threshold(C_GRAPH_DEFINE.ADC_GRAPH)
            # A_adc_graph_object[C_CURVE_DEFINE.MOVING_AVG_STD_HIGH].append(A_i_moving_avg_std_resualt[C_MOVING_AVG_STD_RETURN_DEFINE.TH_HIGH])
            # A_adc_graph_object[C_CURVE_DEFINE.MOVING_AVG_STD_LOW].append(A_i_moving_avg_std_resualt[C_MOVING_AVG_STD_RETURN_DEFINE.TH_LOW])
            
            # A_adc_graph_object[C_CURVE_DEFINE.OTSU_OCCU_CURVE].append(int(0))
            # A_adc_graph_object[C_CURVE_DEFINE.MOVING_STD_OCCU_CURVE].append(int(0))

            # A_adc_graph_object[C_CURVE_DEFINE.OTSU_COUNT_CURVE] = A_i_otsu_resualt[C_OTSU_RETURN_DEFINE.OTSU_COUNT]
            ...
            if len(A_adc_delta_graph_object[C_CURVE_DEFINE.DATA_CURVE]) >= d_WINDOW_SIZE:
                ...
                # for i_curve_index in range(C_CURVE_DEFINE.DATA_CURVE, C_CURVE_DEFINE.OCCU_GRAPH_SIZE, 1):
                for i_curve_index in range(C_CURVE_DEFINE.DATA_CURVE, C_CURVE_DEFINE.CURVE_DEFINE_ARRAY_SIZE, 1):
                    # A_adc_delta_graph_object[i_curve_index] = A_adc_delta_graph_object[i_curve_index][1:d_WINDOW_SIZE]
            ...
            # print("i_adc_delta : ", i_adc_delta)
            # print("i_tp1 : ", i_tp1)
            A_adc_delta_graph_object[C_CURVE_DEFINE.DATA_CURVE].append(i_adc_delta)
            A_adc_delta_graph_object[1] = i_tp1
            # A_adc_delta_graph_object[C_CURVE_DEFINE.MIN_CURVE].append(int(min(A_adc_delta_graph_object[C_CURVE_DEFINE.DATA_CURVE])))
            # A_adc_delta_graph_object[C_CURVE_DEFINE.MAX_CURVE].append(int(max(A_adc_delta_graph_object[C_CURVE_DEFINE.DATA_CURVE])))
            # A_adc_delta_graph_object[C_CURVE_DEFINE.MID_CURVE].append(int(median(A_adc_delta_graph_object[C_CURVE_DEFINE.DATA_CURVE])))
            # A_adc_delta_graph_object[C_CURVE_DEFINE.MEAN_CURVE].append(int(mean(A_adc_delta_graph_object[C_CURVE_DEFINE.DATA_CURVE])))
            # A_adc_delta_graph_object[C_CURVE_DEFINE.MIN_MAX_CENTER_CURVE].append(((int(min(A_adc_delta_graph_object[C_CURVE_DEFINE.DATA_CURVE])) + int(max(A_adc_delta_graph_object[C_CURVE_DEFINE.DATA_CURVE]))) / 2))
            # A_i_otsu_resualt = otsu(C_GRAPH_DEFINE.ADC_DELTA_GRAPH)
            # A_adc_delta_graph_object[C_CURVE_DEFINE.OTSU_CURVE].append(A_i_otsu_resualt[C_OTSU_RETURN_DEFINE.MAX_TH])
            # A_i_moving_avg_std_resualt = moving_avg_std_threshold(C_GRAPH_DEFINE.ADC_DELTA_GRAPH)
            # A_adc_delta_graph_object[C_CURVE_DEFINE.MOVING_AVG_STD_HIGH].append(A_i_moving_avg_std_resualt[C_MOVING_AVG_STD_RETURN_DEFINE.TH_HIGH])
            # A_adc_delta_graph_object[C_CURVE_DEFINE.MOVING_AVG_STD_LOW].append(A_i_moving_avg_std_resualt[C_MOVING_AVG_STD_RETURN_DEFINE.TH_LOW])

            # A_adc_delta_graph_object[C_CURVE_DEFINE.OTSU_OCCU_CURVE].append(int(i_adc_delta >= A_i_otsu_resualt[C_OTSU_RETURN_DEFINE.MAX_TH]))
            # A_adc_delta_graph_object[C_CURVE_DEFINE.MOVING_STD_OCCU_CURVE].append(int(i_adc_delta >= A_i_moving_avg_std_resualt[C_MOVING_AVG_STD_RETURN_DEFINE.TH_HIGH])*2)
            ...
            # A_adc_delta_graph_object[C_CURVE_DEFINE.OTSU_COUNT_CURVE] = A_i_otsu_resualt[C_OTSU_RETURN_DEFINE.OTSU_COUNT]
