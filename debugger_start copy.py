...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        main_h_boxlayout = QHBoxLayout()            # 2. 가로 방향 레이아웃 생성    (자식 위젯들을 좌→우로 배치)
        main_widget.setLayout(main_h_boxlayout)     # 3. 레이아웃 적용
        

        # --- 좌측 패널 (제어 + 설정) ---
        left_widget = QWidget()                     # 1. 대상 위젯 생성
        left_widget.setFixedWidth(300)              # * 위젯 가로 사이즈 설정
        main_h_boxlayout.addWidget(left_widget)     # 1-1. 상위 레이아웃에 위젯 적용
        left_v_boxlayout = QVBoxLayout()            # 2. 세로 방향 레이아웃 생성
        left_widget.setLayout(left_v_boxlayout)     # 3. 레이아웃을 대상 위젯에 적용

        # 5px = 테두리 두께(픽셀).
        # solid = 테두리 스타일 — 실선(draw a solid line). (dashed, dotted, none 등 가능)
        # #3366ff = 테두리 색상(HEX).
        
        # left_widget.setStyleSheet("background-color: #8d5524; border: 1px solid #c68642; color: #FFFFFF;")
        ...
        # left_widget.setStyleSheet("background-color: #77aaff; border-width: 10px; color: #bbeeff;")
        left_label_control = QLabel("제어창")             # 1. 대상 위젯 생성
        # left_label_control.setStyleSheet(f"{SS_PT.format('Segoe UI')} {SS_BOLD} {SS_PT_SIZE.format(14)}")
        # left_label_control.setStyleSheet(f"{SS_BOLD} {SS_PT_SIZE.format(14)}")  # * 위젯 폰트 설정
        left_label_control.setStyleSheet(""
            + SS_BOLD
            + SS_PT_SIZE.format(14)
            )  # * 위젯 폰트 설정
        left_label_control.setFixedHeight(30)             # * 위젯 가로 사이즈 설정
        left_v_boxlayout.addWidget(left_label_control)    # 1-1. 상위 레이아웃에 위젯 적용

#################################################################################################################
        # --- 우측 패널 (그래프 + 로그) ---
        right_widget = QWidget()                    # 1. 대상 위젯 생성
        main_h_boxlayout.addWidget(right_widget)    # 1-1. 상위 레이아웃에 위젯 적용
        right_v_boxlayout = QVBoxLayout()           # 2. 세로 방향 레이아웃 생성
        right_widget.setLayout(right_v_boxlayout)   # 3. 레이아웃을 대상 위젯에 적용
        ...
        # right_v_boxlayout.addWidget(right_placeholder)
#################################################################################################################


        # --- 좌측 패널 구성 ---
        conn_groupbox = QGroupBox("Connection")             # 1. 대상 위젯 생성
        # conn_groupbox.setStyleSheet(f"{SS_BOLD} {SS_PT_SIZE.format(14)}")   # * 위젯 폰트 설정
        left_v_boxlayout.addWidget(conn_groupbox)           # 1-1. 상위 레이아웃에 위젯 적용
        # conn_gridlayout = QGridLayout()                     # 2. Grid 레이아웃 생성
        # conn_groupbox.setLayout(conn_gridlayout)            # 3. 레이아웃을 대상 위젯에 적용

        v_boxlayout_conn = QVBoxLayout()                    # 2. 세로 방향 레이아웃 생성
        conn_groupbox.setLayout(v_boxlayout_conn)            # 3. 레이아웃을 대상 위젯에 적용

        h_boxlayout_port = QHBoxLayout()                    # 2. 가로 방향 레이아웃 생성
        v_boxlayout_conn.addLayout(h_boxlayout_port)            # 3. 레이아웃을 대상 위젯에 적용
        ...
        #     + SS_BOLD
        #     )  # * 위젯 폰트 설정
        label_port.setSizePolicy(QSizePolicy.Policy.Maximum, QSizePolicy.Policy.Maximum)    # 레이블은 텍스트 크기 위주, 늘어나지 않게
        # self.refresh_port_button.adjustSize()
        # # 또는 레이아웃에 힌트 갱신만 알리기
        # self.refresh_port_button.updateGeometry()
        # label_port.setFixedWidth(50)             # * 위젯 가로 사이즈 설정
        #conn_gridlayout.addWidget(label_port, 0, 0)         # 3. 위젯을 대상 레이아웃에 적용 [Grid 행(row)과 열(column)]
        h_boxlayout_port.addWidget(label_port)         # 3. 위젯을 대상 레이아웃에 적용

        self.combo_port = QComboBox()                               # 1. 대상 위젯 생성
        # port_h_boxlayout.addWidget(self.port_combo)                 # 1-1. 상위 레이아웃에 위젯 적용
        # conn_gridlayout.addWidget(self.combo_port, 0, 1)         # 3. 위젯을 대상 레이아웃에 적용 [Grid 행(row)과 열(column)]
        h_boxlayout_port.addWidget(self.combo_port)         # 3. 위젯을 대상 레이아웃에 적용

        # self.refresh_port_button = QPushButton("🔍")              
        self.refresh_port_button = QPushButton("🔍검색")     # 1. 대상 위젯 생성
        # self.refresh_port_button.setStyleSheet(""
        #     + SS_BOLD
        #     )  # * 위젯 폰트 설정
        self.refresh_port_button.setSizePolicy(QSizePolicy.Policy.Maximum, QSizePolicy.Policy.Maximum)  # 레이블은 텍스트 크기 위주, 늘어나지 않게
        # self.refresh_port_button.adjustSize()
        # # 또는 레이아웃에 힌트 갱신만 알리기
        # self.refresh_port_button.updateGeometry()
        # port_h_boxlayout.addWidget(self.refresh_port_button)        # 1-1. 상위 레이아웃에 위젯 적용
        # conn_gridlayout.addLayout(port_h_boxlayout, 0, 1)           # 3. 위젯을 대상 레이아웃에 적용 [Grid 행(row)과 열(column)]
        # conn_gridlayout.addWidget(self.refresh_port_button, 0, 2)         # 3. 위젯을 대상 레이아웃에 적용 [Grid 행(row)과 열(column)]
        h_boxlayout_port.addWidget(self.refresh_port_button)         # 3. 위젯을 대상 레이아웃에 적용


        h_boxlayout_bandrate = QHBoxLayout()                    # 2. 가로 방향 레이아웃 생성
        v_boxlayout_conn.addLayout(h_boxlayout_bandrate)            # 3. 레이아웃을 대상 위젯에 적용
        label_baudrate = QLabel("⚡보드레이트: ")
        label_baudrate.setStyleSheet(
            "border-radius: 2px;"
            )
        label_baudrate.setSizePolicy(QSizePolicy.Policy.Maximum, QSizePolicy.Policy.Maximum)    # 레이블은 텍스트 크기 위주, 늘어나지 않게
        # conn_gridlayout.addWidget(label_baudrate, 1, 0)         # 3. 위젯을 대상 레이아웃에 적용 [Grid 행(row)과 열(column)]
        h_boxlayout_bandrate.addWidget(label_baudrate)         # 3. 위젯을 대상 레이아웃에 적용 [Grid 행(row)과 열(column)]
        self.combobox_baudrate = QComboBox()                               # 1. 대상 위젯 생성
        h_boxlayout_bandrate.addWidget(self.combobox_baudrate)         # 3. 위젯을 대상 레이아웃에 적용
        
        self.connect_button = QPushButton("🔌연결하기")
        v_boxlayout_conn.addWidget(self.connect_button)            # 3. 레이아웃을 대상 위젯에 적용
        self.connect_button.clicked.connect(self.toggle_connection) # 버튼 기능 구현

        # # 레이아웃: Port 라벨 + 콤보박스 + 재검색 버튼

        # conn_gridlayout.addWidget(QLabel("Baud Rate:"), 1, 0)
        # conn_gridlayout.addWidget(self.baud_combo, 1, 1)
        # conn_gridlayout.addWidget(self.connect_button, 2, 0, 1, 2)
        ...
        # conn_gridlayout.addWidget(self.reset_button, 3, 0, 1, 2)
    ...
    #     
    #     left_v_boxlayout.addWidget(settings_group)
    #     left_v_boxlayout.addWidget(tp1_control_group)
    #     left_v_boxlayout.addWidget(tp2_control_group)
    #     left_v_boxlayout.addWidget(tp1_recheck_control_group)
    ...
    #     left_v_boxlayout.addWidget(range_control_group)
    #     left_v_boxlayout.addStretch(1)
    ...
    #     right_v_boxlayout.addWidget(adc_raw_graph_group, stretch=2)       # 1. 상단 그래프 (ADC RAW)
    #     right_v_boxlayout.addWidget(fft_graph_group, stretch=2)            # 2. FFT 스펙트럼
    #     # right_v_boxlayout.addWidget(sw_filter_graph_group, stretch=2)     # (SW 필터) - 비활성화
    #     # right_v_boxlayout.addWidget(hw_filter_graph_group, stretch=2)     # 3. HW 필터 - 비활성화
    #     # right_v_boxlayout.addWidget(sw_filter_2_graph_group, stretch=2)   # 4. SW 필터 (SW_HPF, SW_BPF) - 비활성화
    #     right_v_boxlayout.addWidget(log_group, stretch=1)  # 3. 로그
