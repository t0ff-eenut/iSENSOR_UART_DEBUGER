...
class FFT_Module():
    ...
    def fft(self, inter_A_data:list, inter_f_gain:float = 1.0) -> list:
        """ADC 버퍼에 FFT 적용
        
        Args:
            inter_A_data: ADC 샘플 배열 (예: 300개의 uint16)
            inter_f_gain: 진폭 게인 (표시용, 기본값 1.0)
        ...
        self.A_data         = inter_A_data
        self.i_data_len     = len(self.A_data)
        self.f_gain         = inter_f_gain

        # 1. DC 오프셋 제거
        self.A_f_data               = numpy.array(self.A_data, dtype=numpy.float64)
        self.f_data_avg             = numpy.mean(self.A_f_data)               # 배열의 평균값
        self.A_f_DC_exclusion_data  = self.A_f_data - self.f_data_avg     # 배열 DC 성분 제거
        ...
        self.A_fft_result = numpy.fft.rfft(self.A_f_DC_exclusion_data)
        ...
        if len(self.A_magnitudes) > 1:
            ...
            self.f_peak_freq    = self.A_frequencies[self.i_peak_idx]
            self.f_peak_mag     = self.A_magnitudes[self.i_peak_idx]
        else:
            self.i_peak_idx     = None
            self.f_peak_freq    = None
            self.f_peak_mag     = None
