## 📝 진행 중: PC → ESP32 양방향 UART 통신 구현 (2025-12-08)

### 🎯 목표
PC의 Python GUI에서 ESP32의 TP1(임계값) 값을 UART를 통해 실시간으로 변경하고, 변경된 값이 NVS에 저장되도록 한다.
...
│                         PC (Python GUI)                          │
├─────────────────────────────────────────────────────────────────┤
│  [TP1 Input] ──→ [Send Button] ──→ UART TX                      │
│                                                                  │
│  UART RX ──→ [Frame Parser] ──→ [Graph Update]                 │
└───────────────────────────┬─────────────────────────────────────┘
                            │ UART (1.5Mbps)
                            ▼
┌─────────────────────────────────────────────────────────────────┐
│                          ESP32-C3                                │
├─────────────────────────────────────────────────────────────────┤
│  UART RX ──→ [Command Parser] ──→ [TP1 설정] ──→ [NVS 저장]    │
│                                                                  │
│  [센서 데이터] ──→ [Frame 생성] ──→ UART TX                     │
└─────────────────────────────────────────────────────────────────┘
```
...
| 필드 | 크기 | 설명 |
|------|------|------|
| STX | 3 bytes | `0xAA 0x55 0xCC` |
| CMD_TYPE | 1 byte | 명령 타입 |
| DATA_LENGTH | 2 bytes | 페이로드 길이 |
| PAYLOAD | N bytes | 명령별 데이터 |
| CHECKSUM | 2 bytes | 체크섬 |
| ETX | 3 bytes | `0xDD 0x55 0xAA` |

**명령 타입 (CMD_TYPE):**

| 명령 | 값 | 설명 | 페이로드 |
|------|-----|------|----------|
| CMD_SET_TP1 | 0x10 | TP1 설정 | uint16_t (2 bytes) |
| CMD_SET_TP2 | 0x11 | TP2 설정 | uint64_t (8 bytes) |
| CMD_GET_SETTINGS | 0x20 | 설정값 요청 | 없음 |
| CMD_SAVE_NVS | 0x30 | NVS에 저장 | 없음 |

---

### 📝 구현 단계

#### Phase 1: ESP32 펌웨어 - UART RX 구현

| 작업 | 파일 | 상태 |
|------|------|------|
| 명령 타입 enum 추가 | `shared_protocol.h` | ⬜ |
| RX 스레드 생성 | `custom_esp_uart_rx.c/h` | ⬜ |
| 명령 파서 구현 | `custom_esp_uart_rx.c` | ⬜ |
| main.c에서 초기화 | `main.c` | ⬜ |

#### Phase 2: Python GUI - 송신 기능 추가

| 작업 | 파일 | 상태 |
|------|------|------|
| 명령 상수 추가 | `protocol_config.py` | ⬜ |
| 명령 송신 클래스 | `command_sender.py` | ⬜ |
| TP1 입력 UI | `gui_debugger.py` | ⬜ |
...
- [x] main.c에서 잔여 Queue 모두 소진 후 종료
- [x] Graph에 TP1 임계선 출력
- [x] TP1 초과 지점 표시 (빨간색 X 마커)
