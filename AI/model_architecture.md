...
|------|------|------|
| $x$ | 정규화 전 원본 값 | `3.7` |
| $\mu_{batch}$ | 현재 배치의 **평균값** | `49.5` |
| $x - \mu_{batch}$ | 평균을 빼서 중심을 0으로 이동 | `3.7 - 49.5 = -45.8` |
| $\sigma^2_{batch}$ | 현재 배치의 **분산** (값들이 평균에서 얼마나 퍼져있는가) | `2601.0` |
| $\sqrt{\sigma^2_{batch}}$ | **표준편차** (분산의 제곱근) | `51.0` |
| $\epsilon$ | 분모가 0이 되는 것을 막는 아주 작은 수 | `0.00001` |
| $\hat{x}$ | 정규화 완료된 출력값 | `-45.8 / 51.0 ≈ -0.9` |

```
정규화 전:  [3.7,  102.4,  0.003,  88.1]   ← 중심도 범위도 제각각
평균 0 처리: [-46,   56.7, -53.7,  42.4]   ← 중심을 0으로 이동
분산 1 처리: [-0.9,   1.1,  -1.0,   0.8]   ← 범위까지 통일
```

> **분산(Variance)** = 데이터가 평균에서 **얼마나 흩어져 있는가**를 나타내는 수치.  
> 예시 `[3.7, 102.4, 0.003, 88.1]` 기준:
> 1. 평균: $(3.7 + 102.4 + 0.003 + 88.1) \div 4 \approx 48.6$  
> 2. 각 값이 평균에서 벗어난 거리 제곱: $(-44.9)^2 + (53.8)^2 + (-48.6)^2 + (39.5)^2$  
> 3. 그 평균 = **분산** $\approx 2208$ (표에서 `2601`은 설명용 근사값)  
> 4. 표준편차 = $\sqrt{2208} \approx 47$ → 이걸로 나눠서 범위 통일  
>    - 표준편차 = "데이터가 평균에서 평균적으로 얼마나 떨어져 있는가" → 평균 거리 **47**  
>    - 각 값을 **자신의 평균 거리(47)로 나누면** 상대적 크기만 남음:  
>      $\frac{-44.9}{47} \approx -0.96$,  $\frac{53.8}{47} \approx 1.14$,  $\frac{-48.6}{47} \approx -1.03$,  $\frac{39.5}{47} \approx 0.84$  
